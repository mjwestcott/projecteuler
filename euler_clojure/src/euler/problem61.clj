...
(defn search-permutation
  "Given a permutation of functions representing polygonal types, search for a
  solution, or return nil if not found."
  [poly-funcs]
  ...
  (loop [frontier (vec (for [x (fourdigit-polys (first poly-funcs))] [x]))]
    (let [node (peek frontier)
      ...
      (cond
        ...
        (< z 6)
        (let [children (for [y (fourdigit-polys (nth poly-funcs z))
...
(defn problem61 []
  ...
  (let [polys [octagonal heptagonal hexagonal pentagonal square triangle]
        perms (comb/permutations polys)]
    ;; Return the first non-nil result.
    (first (remove nil? (map search-permutation perms)))))
